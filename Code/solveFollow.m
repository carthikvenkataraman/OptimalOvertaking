...
C2=[...
    ...
    x >= task.L.longsafetymargin + 0.5*task.L.length +...
    task.E.longsafetymargin + 0.5*task.E.length + task.L.timelagrear*vx ... % From Hampus's formulation
    vx <= x/dt ...                                                          % No lower bound set to 0 to allow for backup in aborted lane changes
    y <= task.zone.ymax ...
    vy >= -slip*(vx(2:N)+vL) ...                                            % maybe include speed dependent
    vy <= slip*(vx(2:N)+vL) ...                                             % To only allow right turns, otherwise: task.E.vymax ...
